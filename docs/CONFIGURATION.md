...
**⭐ Recommended for beginners** - Simple, reliable, worldwide.

#### Step 1: Create Bot

1. Open Telegram, search for `@BotFather`
2. Send command: `/newbot`
3. Choose bot name (must end with "bot")
   - Example: `MyTrendRadarBot`, `NewsAlertsBot`
...
2. Send `/start`
3. Bot replies with your User ID = Chat ID

#### Step 3: Configure

**GitHub Secrets (Recommended for forks):**
| Secret Name | Value |
|------------|-------|
| `TELEGRAM_BOT_TOKEN` | `123456789:AAHfiqk...` |
| `TELEGRAM_CHAT_ID` | `987654321` |

**config.yaml (Local deployment):**
```yaml
notification:
  webhooks:
    telegram_bot_token: "123456789:AAHfiqk..."
...
curl -X POST "https://api.telegram.org/bot<BOT_TOKEN>/sendMessage" \
  -d "chat_id=<CHAT_ID>" \
  -d "text=Test from TrendRadar"
```

**Expected result:** Message appears in your Telegram chat.

#### Common Issues

<details>
<summary>❌ Bot doesn't send messages</summary>

1. Verify bot token is correct (no extra spaces)
2. Ensure you sent at least one message to bot first
3. Check Chat ID matches your account
4. Try sending test message manually (see curl command above)
</details>

<details>
<summary>❌ "Chat not found" error</summary>

**Cause:** Wrong Chat ID or haven't started conversation with bot.

**Solution:**
1. Open Telegram
2. Search for your bot by username
3. Click "Start" button
4. Get Chat ID again using Method 1
</details>
...
**⭐ Best for Chinese users** - Integrates with WeChat, widely used in China.

#### Step 1: Create Bot (Mobile)

1. Open WeWork app
2. Enter a group chat (or create one for personal use)
3. Tap **"..."** button (top right)
4. Select **"Message Push"** (消息推送)
5. Tap **"Add"** (添加)
6. Name: `TrendRadar`
...
curl -X POST "<WEBHOOK_URL>" \
  ...
  -d '{"msgtype":"text","text":{"content":"Test from TrendRadar"}}'
```

#### WeChat Integration

After setup in WeWork, messages also appear in WeChat:

1. In WeWork, find **"Push to WeChat"** (推送到微信) option
2. Enable it for the bot
3. Messages now deliver to WeChat Notifications

**💡 Bonus:** You can delete WeWork app from phone, still receive in WeChat!

#### Common Issues

<details>
<summary>❌ "Webhook not found" error</summary>

**Cause:** Webhook was deleted or regenerated.

**Solution:**
1. Go back to WeWork group settings
2. Delete old bot
3. Create new bot
4. Update webhook URL in configuration
</details>
...
**⭐ Best message display** - Rich formatting, user-friendly interface.

#### Two Setup Methods

**Method 1: Bot Command** (Simpler, personal notifications)  
**Method 2: Bot Application** (More complex, group notifications)

We'll cover Method 1 here. For Method 2, see [README-EN.md](../README-EN.md) lines 920-954.

#### Step 1: Create Bot Command

1. Open in browser: https://botbuilder.feishu.cn/home/my-command
2. Click **"New Bot Command"** (新建机器人指令)
3. Click **"Select Trigger"** → Scroll down → **"Webhook Trigger"**
4. Copy the **Webhook Address** (starts with `https://www.feishu.cn/flow/api/trigger-webhook/...`)
...
1. Click **"Select Action"** → **"Send via Official Bot"**
2. Message title: `TrendRadar Trending Monitor`
3. Click **+ button**, select **"Webhook Trigger"**
4. Arrange fields as shown:

**Message content structure:**
```
{{Webhook Trigger.total_titles}}
...
notification:
  webhooks:
    feishu_url: "https://www.feishu.cn/flow/api/trigger-webhook/..."
```
...
<details>
<summary>❌ "System Error" when receiving notifications</summary>

**Cause:** Need to enable Feishu bot application.

**Solution:**
1. Search for your bot in Feishu mobile app
2. Enable the bot application
3. Try sending notification again
</details>

<details>
<summary>❌ Messages too long, getting cut off</summary>

**Cause:** Feishu has message length limits, already handled by batch sending.

**Check configuration:**
```yaml
notification:
  feishu_batch_size: 29000  # Should be set to this
```

If still having issues, reduce monitored platforms or adjust keywords.
</details>
...
**⭐ Popular in Chinese enterprises**

⚠️ **Note:** Bot must be created on PC, not mobile.

#### Step 1: Create Bot (PC Only)

1. Open DingTalk PC client
2. Enter a group chat
3. Click **group settings icon** (⚙️)
4. Scroll down → **"Bot"** (群机器人) → **"Add Bot"** (添加机器人)
5. Select **"Custom"** (自定义)

#### Step 2: Security Settings

**⚠️ Required:** DingTalk mandates security verification.

**Security method:** Custom Keywords

Add keywords that MUST appear in every message:
- English: `Trending` or `TrendRadar`
- Chinese: `热点` or `趋势`

**Why this works:** TrendRadar always includes these words in notifications.

#### Step 3: Complete Setup

1. Check **"I have read and agreed to the service terms"**
2. Click **"Finish"** (完成)
...
curl -X POST "<WEBHOOK_URL>" \
  -H "Content-Type: application/json" \
  -d '{"msgtype":"text","text":{"content":"Trending Test from TrendRadar"}}'
```

**⚠️ Must include keyword!** If message doesn't contain your configured keyword, it will fail.

#### Common Issues

<details>
<summary>❌ "Keywords not found" error</summary>

**Cause:** Message doesn't contain the required keyword.

**Solution:** TrendRadar automatically includes keywords, but verify:
1. Check your keyword setting in DingTalk bot config
2. Ensure it matches: `Trending`, `TrendRadar`, `热点`, or `趋势`
3. Re-create bot if needed
</details>
